...
**计划与审批**：系统把下一步固化为受限执行计划，高风险边界必须人工批准。

**执行与验证**：确定性适配器执行获批计划；证据校验以 SHA-256 绑定输入和结果，失败时关闭而不是猜测成功。

**执行辅助**：Gemini 只根据数据库中最高优先级事实生成行动简报，不直接越权执行。
...
6. 创建证据校验计划；未批准时拒绝运行，批准后执行并把成功或失败结果写回时间线。
...
- 执行适配器使用明确允许列表；当前只提供确定性 SHA-256 证据校验，不发送消息、不提交账号、不付款。
- 证据文本只在计划创建时即时计算摘要，持久化层仅保存哈希和字节数，不保存原文。
...
- 12 项自动化测试覆盖仪表盘、排序、Gemini 防幻觉提示、演示存储以及批准前拒绝执行、成功回写和哈希不匹配失败关闭。
...
- **批准门控执行**：计划、批准、执行和验证各自可观察，结果自动回写事件流。
...
已完成：公开源码、容器化演示、响应式仪表盘、CockroachDB schema、排序引擎、Gemini 约束层、批准门控证据适配器、App Runner 配置、12 项测试。
